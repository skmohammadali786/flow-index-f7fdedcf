...
export type CyclePhase = 'period' | 'ovulation' | 'calm';
...
interface CycleContextType {
  ...
  logPeriodDay: (date: Date, isPeriod: boolean, flowIntensity?: FlowIntensity) => Promise<void>;
  logMood: (date: Date, mood: Mood) => Promise<void>;
  logSymptom: (date: Date, symptom: Symptom) => Promise<void>;
  logNotes: (date: Date, notes: string) => Promise<void>;
  logWaterIntake: (date: Date, glasses: number) => Promise<void>;
  logMedication: (date: Date, medication: Medication) => Promise<void>;
  logSleep: (date: Date, hours: number, quality?: SleepQuality) => Promise<void>;
  logExercise: (date: Date, minutes: number) => Promise<void>;
  logTemperature: (date: Date, temp: number) => Promise<void>;
  getPredictions: () => CyclePrediction | null;
  getStats: () => CycleStats | null;
...
export function CycleProvider({ children }: { children: ReactNode }) {
  ...
  const [currentPhase, setCurrentPhase] = useState<CyclePhase>('calm');
  ...
  useEffect(() => {
    ...
    const todayLog = logs.find(log => log.date === dateStr);

    const isPeriod = todayLog?.isPeriod || isPredictedPeriod(today);
    const isOvulation = isOvulationDay(today) || isInFertileWindow(today);
  ...
  }, [isLoaded, user, logs, cycles, isPredictedPeriod, isOvulationDay, isInFertileWindow]);
  ...
  useEffect(() => {
    ...
    if (currentPhase === 'period') {
       // Period: Soft Red
       root.style.setProperty('--gradient-soft', 'linear-gradient(180deg, hsl(355 70% 98%), hsl(355 70% 94%))');
       root.style.setProperty('--background', '355 100% 99%');
    } else if (currentPhase === 'ovulation') {
       // Ovulation: Bright Yellow
       root.style.setProperty('--gradient-soft', 'linear-gradient(180deg, hsl(60 100% 98%), hsl(50 100% 90%))');
       root.style.setProperty('--background', '60 100% 99%');
    } else if (currentPhase === 'calm') {
       // Calm: Blue #6495ED
       root.style.setProperty('--gradient-soft', 'linear-gradient(180deg, hsl(219 79% 98%), hsl(219 79% 90%))');
       root.style.setProperty('--background', '219 79% 99%');
    }
  }, [currentPhase]);
